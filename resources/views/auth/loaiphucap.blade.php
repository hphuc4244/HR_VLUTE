    ...
    <div class="container-fluid no-padding">
        <section class="content-header">
            ...
            <ol class="breadcrumb">
                ...
                <li class="active">Loại phụ cấp</li>
            </ol>
        </section>
                ...
                <div class="col-xs-12">
                    <div class="box">
                        <div class="box-header with-border">
                            <h3 class="box-title" style="font-weight: bold; ">Danh sách loại phụ cấp</h3>
                        </div>
                        <!-- /.box-header -->
                        <div class="box-body">
                            <table class="table table-bordered" id="table_id">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-warning btn-xem-ls" data="{{$item->maphucap}}" dataten="{{$item->tenphucap}}" >Xem lịch sử</button>
                                        <button class="btn btn-primary btn-sua" data="{{json_encode($item)}}">Sửa</button>
                                        <button class="btn btn-danger btn-xoa" data="{{$item->maphucap}}">Xóa</button>
                                    </td>
                                </tr>
                                @endforeach
                                </tbody>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control them-tenphucap" placeholder="Nhập tên phụ cấp">
                    </div>
                    <div class="form-group">
                        <label>Hệ số phụ cấp</label>
                        <input type="number" step="0.1" min="0" max="10.0" value="0" class="form-control them-hesophucap" placeholder="Nhập hệ số phụ cấp">
                    </div>
                    <div class="form-group">
                        <label>Ghi chú</label>
                        <input type="text" class="form-control them-ghichu" placeholder="Nhập ghi chú nếu có">
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label>Tên phụ cấp</label>
                        <input type="text" class="form-control sua-tenphucap" placeholder="Nhập tên phụ cấp">
                    </div>
                    <div class="form-group">
                        <label>Hệ số phụ cấp</label>
                        <input type="number" step="0.1" min="0" max="10.0" value="0" class="form-control sua-hesophucap" placeholder="Nhập hệ số phụ cấp">
                    </div>
                    <div class="form-group">
                        <label>Ghi chú</label>
                        <input type="text" class="form-control sua-ghichu" placeholder="Nhập ghi chú nếu có">
                    </div>
                </div>
                <div class="modal-footer">
    ...
    <script>
        $(document).ready(function () {
            ...
            $(document).on('click', '.btn-xem-ls', function (){
                ...
                $.ajax({
                    url: "{{action('App\Http\Controllers\LoaiPhuCapController@getLog')}}",
                    type: "GET",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'maphucap': data,
                    ...
                    success: function (result) {
                        $('.tieu-de-ls').text('Lịch sử thay đổi dữ liệu loại phụ cấp ' + dataten );
                        $('.chen-ls').empty();
                        if(result.length > 0) {
            ...
            $(document).on('click', '.btn-sua', function (){
                ...
                $('.sua-tenphucap').val(data.tenphucap);
                $('.sua-hesophucap').val(data.hesophucap);
                $('.sua-ghichu').val(data.ghichu);
                $('.btn-sua-thong-tin').data('ma',data.maphucap);
            ...
            $('.btn-them-thong-tin').click(function () {
                $.ajax({
                    url: "{{action('App\Http\Controllers\LoaiPhuCapController@putDuLieu')}}",
                    type: "PUT",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'tenphucap': $('.them-tenphucap').val(),
                        'hesophucap': $('.them-hesophucap').val(),
            ...
            $('.btn-sua-thong-tin').click(function () {
                $.ajax({
                    url: "{{action('App\Http\Controllers\LoaiPhuCapController@postDuLieu')}}",
                    type: "POST",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'maphucap': $('.btn-sua-thong-tin').data('ma'),
                        'tenphucap': $('.sua-tenphucap').val(),
                        'hesophucap': $('.sua-hesophucap').val(),
            ...
            $('.btn-xoa-thong-tin').click(function () {
                $.ajax({
                    url: "{{action('App\Http\Controllers\LoaiPhuCapController@deleteDuLieu')}}",
                    type: "DELETE",
                    data: {
                        "_token": "{{ csrf_token() }}",
                        'maphucap': $('.btn-xoa-thong-tin').data('ma')
                    },
                    success: function (result) {
                        if (result === "1") {
                            toastr.success("Kết quả", "Thao tác thành công");
